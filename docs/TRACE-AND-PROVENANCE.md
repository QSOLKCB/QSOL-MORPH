...
For `result_determinism_lowering_decisions[]`, `randomness_lowering_decisions[]`, and `failure_behavior_lowering_decisions[]`, any semantic change between source/requested and effective Core contracts requires all three fields. `transition_decision_id` is the stable identity of that exact first-lowering semantic-transition decision. `transition_authorized_by` resolves to an accepted versioned/content-bound `CONTRACT_TRANSITION` rule. `transition_evidence_id` resolves to passing `validation_evidence[]` with `subject_kind = LOWERING_TRANSITION_DECISION` and `subject_id = transition_decision_id`, binding the requested/effective pair, complete owner-qualified source/Core scopes, accepted transition authority, and active context. Its validation event must precede application of the changed Core contract in the shared event-order domain. A rule assertion without passing subject-bound pre-application evidence is unauthorized and fails closed; the transition fields may be absent only when no semantic transition occurred.

The authority and evidence lineage are preserved into each resulting execution-contract record without reusing one singular evidence record for a different subject. Every affected result-determinism scope, randomness scope, or failure-behavior binding receives its own `transition_evidence_id`, bound to that exact execution-scope record under its type-specific subject kind. The target evidence names the same applicable transition rule and includes the lowering decision's `transition_evidence_id` in `related_evidence_ids[]`. One-to-many lowering therefore creates distinct target-scope evidence records that may all cite one lowering-decision evidence record. A target formed from several authorized lowering decisions cites every applicable lowering evidence record. IR hashes, mapping metadata, identical authority labels, or post-application success cannot substitute for this subject-bound evidence chain, and one `validation_evidence_id` must never stand for two different singular subjects.
...
For `core_to_vector_result_determinism_mapping_decisions[]`, `core_to_vector_randomness_mapping_decisions[]`, and `failure_behavior_mapping_decisions[]`, any semantic change between requested and effective contracts requires `transition_decision_id`, `transition_authorized_by`, and `transition_evidence_id`. The decision ID identifies the exact Core→Vector/Dataflow semantic-transition decision. The authority must resolve to the accepted versioned/content-bound `CONTRACT_TRANSITION` rule; the evidence must resolve to passing `validation_evidence[]` with `subject_kind = LOWERING_TRANSITION_DECISION` and `subject_id = transition_decision_id`, binding the exact requested/effective pair, owner-qualified source/lower scopes, accepted authority, and active context. Its validation event must precede application of the changed lower contract. A rule assertion without passing subject-bound pre-application evidence is an unauthorized transition and fails closed. These fields may be absent only when the mapping is semantics-preserving and no contract transition occurred.

Each resulting execution-contract record receives separate subject-bound transition evidence under its own stable scope/binding ID. That target evidence names the applicable transition authority and includes the second-lowering decision's `transition_evidence_id` in `related_evidence_ids[]`; one-to-many mappings create distinct target evidence records that may share the same lowering evidence as a related predecessor, while fused targets cite every applicable lowering evidence record. Reusing the lowering evidence ID itself for the target record is invalid because the lowering decision and execution scope are different singular subjects.
...
- a semantic-changing lowering lacks a stable `transition_decision_id`, subject-bound lowering evidence, or distinct target-scope evidence linked through `related_evidence_ids[]`, or reuses one evidence ID for different singular subjects;
