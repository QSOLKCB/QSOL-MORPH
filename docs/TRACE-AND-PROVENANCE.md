...
backend_selection_decisions[]:
    ...
    selected_backend_implementation_identity?
    target_architecture?
    device?
    selection_policy_id?
...
For every decision whose `decision_status` is final/executable, `selected_backend_implementation_identity` is mandatory whenever the selected backend implementation can affect execution semantics, numeric behavior, failure behavior, device interaction, or produced results. It content-binds the exact backend implementation that actually executes the governed work, including reference/interpreted runtimes, accelerator backends, drivers/runtime stacks, or other implementations that may have no generated artifact or toolchain invocation. A valid identity may be an immutable backend artifact/content hash, a version plus immutable build/content identity, an immutable package/runtime ID, a driver/runtime build identity, or another frozen identity sufficient to distinguish materially different implementations.

`selected_backend_version` remains useful human/audit metadata but is not by itself sufficient when the version label is mutable, ambiguous, or does not uniquely identify implementation content. It may satisfy the implementation-identity requirement only when the frozen backend contract proves that the version value is immutable and uniquely determines the executed implementation. A backend name, family label, mutable tag, package channel, device name, or selection-policy output is never an implementation identity. A non-executable considered/denied decision may omit the implementation identity only when no backend implementation executed and that identity is not material to validating the decision itself.
...
`effect_end_sequence_index` uses the same frozen monotonic event-order domain as authorization and begin. For every `COMPLETED`, `ABORTED_CLEAN`, or `PARTIAL` attempt, termination of the attempt is definite, so `effect_end_sequence_index` is mandatory and must satisfy:

```text
effect_begin_sequence_index < effect_end_sequence_index
```

For `UNKNOWN`, the end index is mandatory whenever the attempt is known to have terminated even though its completion/observability classification remains unknown. It may be absent only when the termination/completion boundary itself cannot be established. When present it must follow begin in the same event-order domain. `NOT_STARTED` has no end event. Generic attempt `sequence_index`, enclosing CARD/lower-operation completion, later failure order, or output publication order cannot substitute for this explicit boundary event. Missing or contradictory end evidence for a definitively terminated attempt fails closed because the validator otherwise cannot establish whether the effect completed/terminated before a successor effect, failure, or publication event.
...
`epistemic_class_binding_ids[]` is always explicit. **Before interpreting the producer-supplied `semantic_class` label**, independently derive the complete applicable representation-qualified class-binding set from `producer_execution_refs[]`, the entered representation, and every traversed class-preservation/mapping relation. Then require the recorded `epistemic_class_binding_ids[]` to equal that complete duplicate-free set for every output, including outputs labeled `UNCLASSIFIED`. The output label never controls whether derivation runs.

If the independently derived applicable class-binding set is nonempty, `semantic_class` must be the exact class justified by that complete set under the frozen class-preservation/composition rules, and every recorded binding ID must resolve to that provenance. A source class may be preserved into a lower binding, but a local output label cannot substitute for, discard, or selectively omit the binding. If several applicable bindings require composition, the frozen composition/mapping rule must establish one coherent output class or the claim fails closed.

Only when the independently derived applicable class-binding set is empty may the output use `semantic_class = UNCLASSIFIED` (or a frozen no-claim equivalent), record an empty `epistemic_class_binding_ids[]`, and omit `evidence_status`. Such an output may still be operationally successful and reproducible; it simply makes no TEST, VALIDATION/PROOF, or other epistemic claim. Fabricating a class binding, copying an unrelated retained class, writing `PROOF` directly into `semantic_class`, or clearing a real retained binding by asserting `UNCLASSIFIED` is invalid.

This contract defines **no implicit declassification/downgrade escape hatch** from an applicable class binding to `UNCLASSIFIED`. If a future frozen model permits an authorized declassification, it must introduce a separately represented, resolvable transition subject with accepted `EPISTEMIC_TRANSITION` authority and passing subject-bound pre-publication evidence; merely emptying `epistemic_class_binding_ids[]`, changing the output label, or omitting `evidence_status` can never encode that transition. Until such a transition record exists, nonempty applicable class bindings and `semantic_class = UNCLASSIFIED` are contradictory and fail closed.
...
`backend_selection_scope_ids[]` is a normative **duplicate-free exact-set attribution**, not an optional list of machinery records chosen by the producer. Independently derive every applicable backend-selection scope from the concrete `producer_execution_refs[]`, their representation-qualified governed scopes, the actual execution path (including interpreted/reference execution with no generated artifact), the final executable selection decisions, and any validated lowering/selection relation needed to associate those producer subjects with machinery. Do not derive applicability from the output's own claimed array or only from `generated_artifact_ids[]` / `machinery_use_record_ids[]`, because a producer may legitimately have neither.
...
The array is empty only when the independently derived applicable set is empty. Every supplied ID resolves to a retained `backend_selection_scopes[]` record that actually governs a material producer path. Extra unrelated scopes fail just as missing producer scopes do. When several concrete producers contribute to one output, derive the duplicate-free union of every applicable backend-selection scope after frozen composition/mapping rules are applied. This check is independent of generated-artifact/protected-use attribution, so an interpreted/reference output cannot be mislabeled with an unrelated backend history merely because no artifact/use ledger exposes the mismatch.

The execution-contract scope arrays are likewise normative **exact-set attributions**, not optional lists of whichever governing records a producer chooses to mention. For each output, derive the complete applicable record set independently for result determinism, numeric behavior, randomness, and failure behavior from `producer_execution_refs[]`, their representation-qualified containment, any retained Semantic DECK/JOB containment, the canonical owner-qualified contracts that apply to those producer paths, and every material Semantic→Core / Core→Vector-Dataflow mapping or frozen deterministic identity-scope reconstruction rule needed to reach the exact lower/backend units and generated artifacts that produced the output. Frozen inheritance, composition, override, and normalization rules determine which source/lower contracts remain materially governing; a producer may not weaken the set by simply omitting an applicable ancestor or mapped lower scope.

Validation then requires exact duplicate-free set equality between these output arrays and the independently derived applicable stable record IDs:
...
`semantic_class = UNCLASSIFIED` is valid only after the independent class-binding derivation proves the applicable set is empty. It therefore requires an empty class-binding set and no `evidence_status`; the producer's label cannot cause the set to become empty. It cannot be used as a back door to shed a retained TEST/VALIDATION/PROOF binding or to publish an evidence-bearing claim. A distinct validation/proof output may establish its own valid class binding/evidence subject, but it does not retroactively relabel another output. Missing, wrong-kind, stale, unavailable, failed, context-mismatched, different-output, or post-publication rule/evidence rejects the claim.
...
- the independently derived applicable epistemic-class binding set and recorded `epistemic_class_binding_ids[]` differ, including when the producer labels the output `UNCLASSIFIED`;
- an output with a nonempty independently applicable class-binding set reports `UNCLASSIFIED`, clears those binding IDs, or otherwise sheds class provenance without a separately defined authorized declassification transition;
...
- a final executable backend-selection decision lacks a content-bound immutable `selected_backend_implementation_identity` adequate to distinguish the actual executed backend implementation, or relies only on a backend name, mutable version/tag, package channel, device label, or selection-policy result;
...
- an output's `backend_selection_scope_ids[]` differs from the independently derived complete duplicate-free set governing its concrete producer execution refs, including an unrelated extra scope or a missing producer scope even when no generated artifact or machinery-use record exists;
...
| Direct Core output class | A direct Core output always has its concrete producer. The complete applicable class-binding set is derived before reading the output label. If the entered Core IR has an applicable representation-qualified epistemic binding, `epistemic_class_binding_ids[]` equals that complete set and `semantic_class` is the exact justified class. Only if the derived set is empty is the output `UNCLASSIFIED` with an empty binding set and no evidence claim. | Fabricated Semantic class lineage; arbitrary `TEST`/`VALIDATION`/`PROOF`; unrelated lower binding; operational success treated as class provenance; `UNCLASSIFIED` used to clear a retained TEST/VALIDATION/PROOF binding. |
...
| Effect boundary ordering | Every `COMPLETED`, `ABORTED_CLEAN`, `PARTIAL`, or `UNKNOWN` attempt has a concrete begin index; its linked authorization is GRANTED, has a concrete authorization index in the same event-order domain, and the authorization index precedes begin. Every `COMPLETED`, `ABORTED_CLEAN`, or `PARTIAL` attempt also has a concrete end index with begin before end; `UNKNOWN` records end whenever definite termination is known and may omit it only when termination/completion cannot be established. | Begun state with missing begin index; GRANTED authorization with missing ordering index; late authorization; completed/clean-abort/partial attempt with missing end; end before/equal begin; known-terminated UNKNOWN without end; generic attempt sequence used as boundary proof; denied authorization paired with a begun state. |
