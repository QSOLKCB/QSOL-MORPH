...
# Canonical backend governed scope: stable ledger key + qualified computation identity.
rep(
...
# Canonical execution-contract scopes: every governed computation is owner-qualified.
rep(
    "docs/TRACE-AND-PROVENANCE.md",
...
# Canonical DECK ledger: separate a DECK-caused failure from a prior failure that blocks start.
rep(
    "docs/TRACE-AND-PROVENANCE.md",
    """deck_executions[]:
    deck_execution_id
...
# Reproducibility mirrors the qualified governed scopes.
rep(
...
# Backend-domain projection mirrors governed_scope_ref.
rep(
...
# Failure-domain projection mirrors governing failure cause.
rep(
...
# PR #5 gate: qualified governed scopes.
rep(
    "ROADMAP.md",
...
# PR #5 gate: direct input consumer/acquisition joins.
rep(
    "ROADMAP.md",
...
# PR #5 outputs: exact protected-use occurrence relation.
rep(
    "ROADMAP.md",
...
# PR #5 toolchain gate: exact IR inputs.
rep(
    "ROADMAP.md",
...
rep(
    "ROADMAP.md",
    """Toolchain `input_ids[]` bind the immutable bytes of prebuilt objects, libraries, headers, startup files, sysroots, and implicit dependencies not generated in this run.
""",
...
# Lossless serialization: dependencies use owner-qualified producer/result references.
rep(
...
# Canonical Semantic IR defines the same dependency reference, so serialization is not stronger than the model.
rep(
    "docs/SEMANTIC-IR.md",
...
# Human/AI conceptual CARD mirrors the structured dependency meaning.
rep(
...
# Agent-facing mirrors for governed scope qualification and DECK blocking cause.
rep(
    "AGENTS.md",
...
rep(
    "AGENTS.md",
    """Every selected DECK remains represented even if prior fail-stop prevents it from starting. Every CARD in a selected DECK execution receives an outcome/path record.
""",
...
# README overview mirrors direct input consumers and qualified governed scopes.
rep(
    "README.md",
...
print("all requested/projection fixes staged")
